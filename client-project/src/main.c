...
int main(int argc, char *argv[]) {
    ...
    // Copia città e termina stringa
    memset(request.city,0,sizeof(request.city));
    ...
    if (sendto(sock, buffer, sizeof(buffer), 0, (struct sockaddr*)&server_addr, sizeof(server_addr)) != sizeof(buffer)) {
        perror("sendto"); closesocket(sock); clearwinsock(); return 1;
    }
    ...
    if (response.status==STATUS_OK) {
        switch(response.type) {
            case TYPE_TEMP:  printf("Temperatura = %.1f°C\n", response.value); break;
            case TYPE_HUM:   printf("Umidità = %.1f%%\n", response.value); break;
            case TYPE_WIND:  printf("Vento = %.1f km/h\n", response.value); break;
            case TYPE_PRESS: printf("Pressione = %.1f hPa\n", response.value); break;
